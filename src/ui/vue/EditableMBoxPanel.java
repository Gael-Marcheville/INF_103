...
public class EditableMBoxPanel extends JButton {
	...
	/**
	 * Retourne un objet EditableMBoxPanel, Box d'un Maze lorsque le mode
	 * construction est activé
	 * 
	 * @param mazeApp
	 * @param color
	 * @param mBox
		...
		this.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				try {
					...
					if (mBox instanceof EBox && mazeAppModel.getBuildModeType() == "Wall") {
						mazeAppModel.setBox(X, Y, new WBox(X, Y, currentMaze));
					} else if (mBox instanceof WBox && mazeAppModel.getBuildModeType() == "Empty") {
						mazeAppModel.setBox(X, Y, new EBox(X, Y, currentMaze));
